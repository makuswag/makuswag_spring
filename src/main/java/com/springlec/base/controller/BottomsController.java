...
import com.springlec.base.service.BottomsDaoService;

@Controller
public class BottomsController {
	@Autowired
	BottomsDaoService service;
	
	@GetMapping("bottoms")
	public String list(@RequestParam(defaultValue = "1") int page, Model model) throws Exception {
	    int numOfTuplePerPage = 9; // 페이지당 튜플 개수
	    List<BottomsDto> listDao = service.listDao(page, numOfTuplePerPage); // 서비스 계층에서 데이터 조회
