...
import com.springlec.base.service.TopShortDaoService;

@Controller
public class TopShortController {
	
	@Autowired
	TopShortDaoService service;
	
	@GetMapping("topshort_sleeve")
	public String list(@RequestParam(defaultValue = "1") int page, Model model) throws Exception {
	    int numOfTuplePerPage = 9; // 페이지당 튜플 개수
	    List<TopShortDto> listDao = service.listDao(page, numOfTuplePerPage); // 서비스 계층에서 데이터 조회
